...
can scan the package on VirusTotal with your own key, and opens an issue here from your GitHub
account. The issue carries the one file the catalog will hold for your module,
`mods/<module id>.ltx`:
...
- **Waiting** - for VirusTotal to finish a scan, for a service to answer, or for the next day
  when the day's new modules are listed already. The catalog looks again every half hour;
  submitting again makes it look at once.
...
- Updates need no submission: publish a new release with XFined Editor and the browser picks it
  up within six hours.
